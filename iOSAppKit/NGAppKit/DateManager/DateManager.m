...
- (instancetype)initWithStyle:(NSDateFormatterStyle)format{
    if (self = [super init]) {
        self.formatter = [[NSDateFormatter alloc] init];
        [self.formatter setDateStyle:format];
        [self.formatter setTimeStyle:format];
        self.calender = [NSCalendar currentCalendar];
    }
    return self;
}
...
- (void)switchToUTC{
    ...
    [self.calender setTimeZone:[NSTimeZone timeZoneWithAbbreviation:@"UTC"]];
}

- (void)switchToLocal{
    [self.formatter setTimeZone:[NSTimeZone localTimeZone]];
    [self.calender setTimeZone:[NSTimeZone localTimeZone]];
}
...
- (NSInteger) count:(DateComponentType)type from:(NSDate*)from{
    ...
    //NSLog(@"%@",[self.formatter stringFromDate:[self now]]);
    ...
    [self.calender rangeOfUnit:unit startDate:&toDate
                      interval:NULL forDate:[self now]];
    
    //NSLog(@"From: %@",[self.formatter stringFromDate:fromDate]);
    //NSLog(@"To: %@",[self.formatter stringFromDate:toDate]);
...
- (NSDate*) tomorrow{
    NSDateComponents *component = [self component:Day forValue:1];
    return [self.calender nextDateAfterDate:[NSDate date] matchingComponents:component options:0];
}
